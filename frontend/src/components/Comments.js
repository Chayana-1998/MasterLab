...
const Comments = ({ experimentId, token }) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const response = await axios.get(`http://localhost:1234/comments/${experimentId}/comments`);
    ...
    const handleAddComment = async () => {
        ...
        try {
            const response = await axios.post(`http://localhost:1234/comments/${experimentId}/comments`, { text: newComment }, {
